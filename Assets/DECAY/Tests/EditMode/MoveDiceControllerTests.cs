...
namespace Decay.Tests
{
    public sealed class MoveDiceControllerTests
    {
        ...
        [Test]
        public void EnemySetup_RejectsPlayerMovementBecausePhaseBelongsToEnemy()
        {
            var fixture = new MovementFixture(setupPhase: BattlePhase.EnemySetup);
        ...
        [Test]
        public void PlayerSetup_RejectsEnemyMovementBecausePhaseBelongsToPlayer()
        {
            var fixture = new MovementFixture();
        ...
        private sealed class MovementFixture
        {
            internal MovementFixture(
                ...
                BattlePhase setupPhase = BattlePhase.PlayerSetup)
            {
                BattleState = DiceTestFactory.CreateBattleState();
                if (setupPhase == BattlePhase.PlayerSetup)
                {
                    // Most movement tests exercise player Setup. Move the fixture through the same explicit
                    // phase value used by production architecture; there is no Setup sub-turn state.
                    new AdvancePhaseCommand(BattleState, BattlePhase.PlayerSetup).Execute();
                }
                else if (setupPhase != BattlePhase.EnemySetup)
                {
                    throw new System.ArgumentOutOfRangeException(nameof(setupPhase));
                }
