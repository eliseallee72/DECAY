...
namespace Decay.Tests
{
    public sealed class BattleRuntimePresentationTests
    {
        ...
        [UnityTest]
        public IEnumerator HourglassView_RequestRollUsesBattleFlowAndAnimationCompletionHook()
        {
            PresentationFixture fixture = CreateFixture(primaryRolls: new[] { 4 }, enemyDiceCount: 0);
            DiceInstanceId playerDiceId = new DiceInstanceId(1);
            Assert.That(fixture.Root.RequestPlayerMove(
            ...
            GameObject cameraObject = Track(new GameObject("HourglassCamera_Test"));
            Camera camera = cameraObject.AddComponent<Camera>();
            GameObject hourglassObject = Track(new GameObject("Hourglass_Test"));
            BoxCollider collider = hourglassObject.AddComponent<BoxCollider>();
            HourglassView hourglass = hourglassObject.AddComponent<HourglassView>();
            hourglass.ConfigureForTests(camera, collider, fixture.Root);

            BattleFlowResult roll = hourglass.RequestRoll();

            Assert.That(roll.IsApproved, Is.True);
            Assert.That(fixture.Root.Runtime.BattleState.CurrentPhase, Is.EqualTo(BattlePhase.Rolling));
            Assert.That(fixture.Root.Runtime.BattleInventoryState.GetDice(playerDiceId).CurrentFaceIndex, Is.EqualTo(4));

            hourglass.NotifyRollPresentationComplete();
            Assert.That(fixture.Root.Runtime.BattleState.CurrentPhase, Is.EqualTo(BattlePhase.EnemyReposition));
            yield return null;
        }
