...
namespace Decay.Tests
{
    public sealed class BattleRuntimePresentationTests
    {
        ...
        [UnityTest]
        public IEnumerator Initialize_EnemySetupPlacesDiceBeforeViewsAndHidesUnplacedEnemyInventory()
        {
            PresentationFixture fixture = CreateFixture(enemyDiceCount: 7);

            for (int slotNumber = 1; slotNumber <= 6; slotNumber++)
            {
                DiceInstanceId expectedDiceId = new DiceInstanceId(slotNumber + 1);
                SlotId slotId = new SlotId(Side.Enemy, slotNumber);
                Assert.That(fixture.Root.Runtime.BoardState.GetSlot(slotId).OccupantDiceId, Is.EqualTo(expectedDiceId));
                Assert.That(GetRequiredView(fixture.Root, expectedDiceId).SpriteRenderer.enabled, Is.True);
            }

            DiceInstanceId unplacedEnemyId = new DiceInstanceId(8);
            Assert.That(fixture.Root.Runtime.BattleInventoryState.IsInInventory(unplacedEnemyId), Is.True);
            DiceView unplacedEnemyView = GetRequiredView(fixture.Root, unplacedEnemyId);
            Assert.That(unplacedEnemyView.SpriteRenderer.enabled, Is.False);
            Assert.That(unplacedEnemyView.GetComponent<Collider>().enabled, Is.False);
            yield return null;
        }
        ...
        public IEnumerator Initialize_InventoryViewsUsePresentationLayoutWithoutOwningMembership()
        {
            ...
            Assert.That(playerView.transform.position, Is.EqualTo(fixture.Layout.GetPlayerInventoryDicePosition(0, 1)));
        ...
        public IEnumerator RejectedMovement_ReconcilesDraggedViewBackToAuthoritativeLocation()
        {
            ...
            Assert.That(move.IsRejected, Is.True);
            Assert.That(fixture.Root.Runtime.BattleInventoryState.IsInInventory(playerDiceId), Is.True);
        ...
        {
            PresentationFixture fixture = CreateFixture(primaryRolls: new[] { 4 }, enemyDiceCount: 0);
            DiceInstanceId playerDiceId = new DiceInstanceId(1);
        ...
        [UnityTest]
        public IEnumerator DiceView_VisualVisibilityDoesNotOwnInteractionCollider()
        {
            Sprite sprite = CreateSprite();
            DiceView view = CreateDiceViewPrefab(sprite);
            Collider collider = view.GetComponent<Collider>();
            Assert.That(collider.enabled, Is.True);

            view.SetVisualContent(sprite, false);

            Assert.That(view.SpriteRenderer.enabled, Is.False);
            Assert.That(collider.enabled, Is.True, "Renderer visibility must not implicitly own the interaction surface.");
            yield return null;
        }

        [UnityTest]
        public IEnumerator DiceView_DestinationCanChangeWithoutMovingRenderedTransformUntilReconcile()
        {
            Sprite sprite = CreateSprite();
            DiceView view = CreateDiceViewPrefab(sprite);
            Transform anchor = CreateAnchor("SemanticDestination_Test", new Vector3(7f, 3f, -2f));
            Vector3 renderedBefore = new Vector3(-4f, 1f, 9f);
            view.transform.position = renderedBefore;

            view.SetPresentationDestination(new DicePresentationDestination(anchor, Vector3.zero));

            Assert.That(view.transform.position, Is.EqualTo(renderedBefore));
            Assert.That(view.PresentationDestinationWorldPosition, Is.EqualTo(anchor.position));

            view.ReconcileRenderedTransformToDestination();
            Assert.That(view.transform.position, Is.EqualTo(anchor.position));
            yield return null;
        }
        ...
        {
            PresentationFixture fixture = CreateFixture(primaryRolls: new[] { 6 }, enemyDiceCount: 0);
            DiceInstanceId playerDiceId = new DiceInstanceId(1);
            DiceView playerView = GetRequiredView(fixture.Root, playerDiceId);
            ...
            Assert.That(fixture.Root.Runtime.BattleState.CurrentPhase, Is.EqualTo(BattlePhase.Setup));
            yield return null;
        }

        [UnityTest]
        public IEnumerator HourglassView_InteractionRequestsAuthoritativeFlowWithoutOwningPhaseProgression()
        {
            ...
            fixture.Hourglass.NotifyInteractionRequested();

            // With no authored Animator bindings, presentation completion reports immediately and the authoritative
            // bridge proceeds through EnemyReposition to PlayerReposition. The View itself never calls BattleController.
        ...
        private PresentationFixture CreateFixture(int[] primaryRolls = null, int enemyDiceCount = 1, bool withPresentationDirector = false)
        {
            ...
            if (withPresentationDirector)
            {
                ...
                hourglass = hourglassObject.AddComponent<HourglassView>();
                hourglass.ConfigureForTests(presentationCamera, hourglassCollider);
            ...
            var enemySeeds = new List<DiceRuntimeSeed>(enemyDiceCount);
            for (int i = 0; i < enemyDiceCount; i++)
            {
                enemySeeds.Add(DiceRuntimeSeed.FromDefinition(definition));
            }
            ...
            return new PresentationFixture(root, layout, diceViewRoot, neutralSprite, faceSprites, presentationDirector, hourglass);
        }
        ...
        private BattleSceneDiceLayout CreateLayout()
        {
            ...
            layout.ConfigureForTests(slotAnchors, playerInventory, dropCollider);
        ...
        private sealed class PresentationFixture
        {
            ...
            public HourglassView Hourglass { get; }
        }
    }
}
