...
namespace Decay
{
    ...
    public sealed class DecayResolver
    {
            ...
            DecaySaveToken? playerPendingSave)
        {
            ...
            DecaySideDecision enemyDecision = BuildSideDecision(
                enemy,
                enemySource,
                playerSource,
                playerSource ? player.DiceId : default,
                playerSource ? player.SlotId : default,
                enemyPendingSave);
            DecaySideDecision playerDecision = BuildSideDecision(
                player,
                playerSource,
                enemySource,
                enemySource ? enemy.DiceId : default,
                enemySource ? enemy.SlotId : default,
                playerPendingSave);
    ...
    internal readonly struct DecaySideDecision
    {
            ...
            DecaySaveToken? saveUsed)
        {
            if (isTargeted)
            {
                if (!targetingDiceId.IsValid || !targetingSlotId.IsValid)
                    throw new InvalidOperationException("TARGETED DECAY decisions require the opposing 6 source identity.");
                if (targetingSlotId != snapshot.SlotId.Opposing)
                    throw new InvalidOperationException("A TARGETED DECAY source must occupy the opposing slot in the same pair snapshot.");
            }
            else if (targetingDiceId.IsValid || targetingSlotId.IsValid)
            {
                throw new InvalidOperationException("Non-TARGETED DECAY decisions cannot carry an opposing targeting source.");
            }
