...
namespace Decay
{
    ...
    /// they never establish gameplay occupancy or inventory membership.
    /// </summary>
    public sealed class BattleSceneDiceLayout : MonoBehaviour
    {
        ...
        [Header("Battle Inventory Presentation")]
        ...
        [SerializeField] private Transform _enemyInventoryAnchor;
        [SerializeField] private Vector3 _enemyInventorySpacing = new Vector3(1.45f, 0f, 0f);
        ...
        public Vector3 GetInventoryDicePosition(Side side, int displayIndex, int displayCount)
        {
            ...
            Transform anchor = side == Side.Player ? _playerInventoryAnchor : _enemyInventoryAnchor;
            if (anchor == null)
            {
                throw new InvalidOperationException($"{side} Battle Inventory presentation anchor is not configured.");
            }

            Vector3 spacing = side == Side.Player ? _playerInventorySpacing : _enemyInventorySpacing;
            float centeredIndex = displayIndex - ((displayCount - 1) * 0.5f);
            return anchor.position + (spacing * centeredIndex);
        }
        ...
        public bool TryValidate(out string error)
        {
            ...
            if (_enemyInventoryAnchor == null)
            {
                error = "Enemy Battle Inventory presentation anchor is missing.";
                return false;
            }
        ...
        internal void ConfigureForTests(
            ...
            Transform enemyInventoryAnchor)
        {
            ...
            _enemyInventoryAnchor = enemyInventoryAnchor;
            _isIndexed = false;
        }
