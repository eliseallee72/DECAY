...
namespace Decay
{
    ...
    public sealed class BattleBootstrapper
    {
        public BattleRuntime Create(
            ...
            IEnemySetupPlanner enemySetupPlanner = null)
        {
            ...
            var enemyResetEntries = new List<KeyValuePair<DiceInstanceId, DiceRuntimeSeed>>(enemySeeds.Count);
            ...
            {
                var instanceId = new DiceInstanceId(nextInstanceId++);
                runtimeDice.Add(DiceRuntimeState.CreateEnemyDice(instanceId, enemySeeds[i]));
                enemyResetEntries.Add(new KeyValuePair<DiceInstanceId, DiceRuntimeSeed>(instanceId, enemySeeds[i]));
            }
            ...
            var enemyResetSeeds = new EnemyDiceResetSeedCatalog(enemyResetEntries);
            ...
            var enemyController = new EnemyController(
                ...
                battleInventoryState,
                moveDiceController,
                enemySetupPlanner ?? new FillAvailableEnemySlotsPlanner());
            ...
            var decayExecutor = new DecayExecutor(
                ...
                battleInventoryState,
                history);
            var scoreExecutor = new ScoreExecutor(
                battleState,
                boardState,
                battleInventoryState,
                scoreState,
                history);
            var roundEndExecutor = new RoundEndExecutor(
                battleState,
                boardState,
                battleInventoryState,
                scoreState,
                history);
            var gameEndExecutor = new GameEndExecutor(
                battleState,
                boardState,
                battleInventoryState,
                globalInventory,
