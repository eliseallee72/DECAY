...
namespace Decay
{
    ...
    internal sealed class DecayDiceCommand
    {
        ...
        private readonly DiceInstanceId _targetingDiceId;
        private readonly SlotId _targetingSlotId;
            ...
            SlotId targetingSlotId)
        {
            ...
            if (wasTargeted)
            {
                if (!targetingDiceId.IsValid) throw new ArgumentException("TARGETED DECAY requires a valid opposing 6 dice ID.", nameof(targetingDiceId));
                if (!targetingSlotId.IsValid) throw new ArgumentException("TARGETED DECAY requires a valid opposing 6 slot ID.", nameof(targetingSlotId));
                if (targetingSlotId != slotId.Opposing)
                    throw new ArgumentException("The targeting 6 must come from the opposing slot in the same pair.", nameof(targetingSlotId));
            }
            else if (targetingDiceId.IsValid || targetingSlotId.IsValid)
            {
                throw new ArgumentException("Non-TARGETED DECAY cannot carry an opposing targeting source.", nameof(targetingDiceId));
            }
            _diceId = diceId;
            _slotId = slotId;
            _wasWillDecay = wasWillDecay;
            _wasTargeted = wasTargeted;
            _targetingDiceId = targetingDiceId;
            _targetingSlotId = targetingSlotId;
        }
