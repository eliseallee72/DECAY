...
- Canonical validated baseline: GitHub `step-6` (Unity-validated by Elise).
- Current local implementation pass: Step 7 first pass built from accepted `step-6`; no Step 7 GitHub branch is created during local validation.
...
- Step 7 first pass: DecayResolver + DecayExecutor, process-local ordered WILLSAVE identity, 1..6 simultaneous pair decisions, DECAYED/SAVED outcomes, Broken/Unstable propagation, DECAY Facts, completion receipt/gate, and BattleController `RequestDecay -> CompleteDecay -> ScoreProcess` orchestration. Unity validation pending.
...
- Step 7 now resolves the full logical DECAY pass and intentionally stops after entering ScoreProcess; Score remains Step 8.
...
- Logical runtime dice creation/view binding and baseline Player drag/drop are validated from Step 6. Logical Decay/Save resolution is implemented in the Step 7 first pass. Roll/Decay/Save animation presentation, EnemyController planning, Score, RoundEnd/GameEnd cleanup, Drawer/carousel presentation, and permanent save/upgrade semantics remain deferred.
...
- Step 7 first pass: expected 209 EditMode + 7 PlayMode after import; Unity compilation/Test Runner validation pending.
