...
- Baseline branch: GitHub `step2` (tree `1e8fa9180fa6c9812f4b3cf7cb62b7a6f62b0b90`), validated by Elise with 102/102 EditMode tests green and successful Play Mode startup before Step 3 began.
...
- Step 3 first pass implemented locally: `MoveDiceRequest`/target/result, authoritative movement Gate context, narrow core movement Gates, and one `MoveDiceController.RequestMove` path that maps approved Setup/Reposition requests to the existing Step 2 Commands and records Facts.
- Step 3 remains EditMode-focused. Runtime dice spawning/state-to-view binding and real click/drag adapters are not implemented yet, so no disposable PlayMode drag harness was added. Processing/tutorial/blocking Gates remain deferred until their authoritative state owners exist.
...
- Step 3 Unity compilation/Test Runner is unverified until this package is opened in Unity 6000.5.8f1.
- Setup actor sequencing, Processing/Tutorial/blocking Gate owners, runtime dice view/input wiring, Decay/GameEnd membership orchestration, slot process transitions, and permanent-upgrade/save commit rules remain intentionally deferred; see the running Foundation Change Tracker.
