...
- Last analyzed commit: `f63256d` (`main`).
...
- Step 1 foundation revised locally: typed stable content IDs, unique dice/effect runtime instance IDs, `BattleConfig`, `DiceDefinition`, `DiceFaceDefinition`, `EffectDefinition` base, `DiceCatalog`, runtime seeds/state, explicit slot-pair identity, `BattlePhaseTransitionValidator`, and the initial structured `BattleHistory` foundation.
- Step 2 next: `BattleState`, `SlotState`, `BoardState`, `GlobalInventoryState`, and `BattleInventoryState`, still without final visuals. Board/inventory membership must become authoritative there; `DiceRuntimeState` intentionally does not own location.
...
- Unity compilation and Test Runner results are unverified until the project is opened in Unity 6000.5.8f1.
- The precise permanent-upgrade commit rules remain intentionally deferred.
